...
ACIA_RX_IRQ: 										; IRQ handler for ACIA
 ...
 AND %10000000
 jp nz, IRQ_Return 							; 	If not, check COM2
...
MON_READ_LINE_LOOP_2:
 ...
 cp $20                                 ;   If Byte >= $20
 jp nc,MON_READ_LINE_4                  ;   its a character
...
MSG:
 ...
 .db $0D ; carriage return
 .db $0A ; line feed
 .db "32,511 BYTES FREE [Snapshot 14/04/22a]"
READY:
 .db $0D ; carriage return
 .db $0A ; line feed
 .db "READY"
...
BinToASCII: ; this subroutine converts a binary number into an ascii-based hex number, ready to be displayed : a=input, de = output
 ...
 AND $0F  ;-> look at the first nibble 
 ld hl,HEX_TABLE
 add a,l
 ld l,a
 ld d,(hl)
